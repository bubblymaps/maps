...
export function MapScale({ map, maxWidth = 100 }: MapScaleProps) {
  ...
  useEffect(() => {
    ...
    const updateScale = () => {
      ...
      setScaleLabel(
        niceScale >= 1000 ? `${niceScale / 1000} km` : `${niceScale} m`
      );
    };
  ...
  return (
   <div className="fixed left-5 bottom-5 z-10 flex flex-nowrap items-center gap-2 text-xs font-medium text-foreground select-none">
      ...
      <span className="whitespace-nowrap">{scaleLabel}</span>
    </div>
  );
}
